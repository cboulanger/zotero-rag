...
Found during final holistic review across all commits; assessed as narrow/bounded and not worth the added complexity to close fully:

- **Startup-lag race between triggering a run and the client's redirect check.** `POST /api/autoindex/run` returns `{"started": true}` as soon as the subprocess is spawned — before that subprocess has re-validated every registered user's Zotero key (`resolve_targets()`, one network call per registered key) and acquired its lock, which can take a few seconds. If the *same* user clicks "Run now" and then, within that window, also clicks "Index" in the search dialog, `isServerIndexingRunning()` (§6) can still observe `running: false` and let client-side indexing proceed. Closing this fully would require either blocking the HTTP response until the subprocess confirms it's running (adds latency, defeats the fire-and-forget design) or having the endpoint pre-write a "starting" status itself (risks a status file stuck at `running: true` forever if the subprocess then fails to start before writing its own accurate status). Given the narrow, same-user, closely-timed trigger condition required, and that no protection against this class of client-vs-server race existed at all before this feature, this is accepted as-is rather than fixed.
- **Rate-limited embedding keys get inconsistent wording across UI surfaces.** `_embedding_key_block_reason()` (§2) gives a specific, correct message for a `rate_limited` embedding key. `plugin/src/preferences.js`'s three status-message call sites have no dedicated `rate_limited` branch and fall through to a generic "unexpected status" message for the same state. This gap pre-dates this plan (it's in the per-user-embedding-key feature `preferences.js` was built in) and touching `preferences.js` was out of scope here — noted for a possible follow-up, not fixed in this plan.
