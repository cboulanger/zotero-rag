...
- `POST /api/chapter-linking/review/{queue_id}/approve` — body: `library_slug`, plus optional edits (`pdf_start_index`/`pdf_end_index`/`title`/`authors` for a `chapter` entry, `book_key` — the chosen candidate — for a `match` entry). Used by the "Needs Review" view's ✓/✎ actions on a single `bucket: "review"` entry. Dispatches by `type`:
  ...
  - `ocr` → starts an OCR job for that attachment (reusing `ocr_attachments.py`'s per-item logic) and, on completion, re-runs analysis (`analyze_book_chapters.py`'s logic) for that book, clearing the `ocr` queue entry either way. **Correction from the original spec text**: analysis alone never produces a `chapter` entry — chapter classification into review/commit buckets is `upload_chapters.py`'s job, a separate step with its own `confidence_threshold`/`target_collection` that the `ocr` entry's payload doesn't carry. So the actual outcome is: the attachment either now has an extractable text layer (analysis proceeds normally; if it's still not analyzable for some other reason it's re-upserted as `needs_ocr` again) or it doesn't (re-upserted as `needs_ocr` still) — either way, turning a freshly-OCR'd book into `chapter`/`match` queue entries requires a separate, manual `upload_chapters.py --dry-run` (or `POST .../segment-upload`) pass over it afterward. See `docs/chapter-segmentation.md`'s "Known limitation" note.
...
Chaining a successful OCR-approve directly into a `chapter_upload.py --dry-run` pass (so "Run OCR" can end in fresh `chapter`/`match` queue entries, closing the loop §4's original text incorrectly assumed already existed) is also left for a future iteration — it needs the `ocr` entry's payload extended with `confidence_threshold`/`target_collection` so the auto-chained dry-run knows what to use, which is a small but deliberate schema change, not something to bolt on silently.
