...
| A metadata field is cleared entirely (last tag removed, all authors deleted, title blanked) | Not pushed live — the endpoint's conditional-field pattern (deliberately chosen over the delegation approach, see §5.4) treats an empty value as "field omitted, don't touch it," since it can't distinguish "caller didn't send this field" from "caller cleared it." The hourly cron reconciles it within the hour via the full-item comparison path, which does send explicit empty values. |
...
## 7.1 Implementation deviations found during review

Four issues were found and fixed during task-by-task code review and manual end-to-end verification, after this spec was written and approved. None change the architecture; they're noted here so this document stays trustworthy for anyone resuming work from it, since §5.1/§5.2's code sketches below predate these fixes:

1. **Stale-key race in `TaskQueue._dispatchNext()`.** The original sketch snapshotted a batch but left it in the per-type ready map while awaiting the dispatcher; a fresher edit to the same key arriving mid-flight would get silently overwritten and lost once the stale dispatch resolved. Fixed by removing the batched keys from the ready map *before* awaiting, then re-adding only what didn't succeed and wasn't superseded by a fresher entry.
2. **No per-item error isolation in the modify-notifier loop** (`zotero-rag.js`). One item's exception (e.g. from `getBackendLibraryId`) could abort processing of every other id in the same notifier batch. Fixed with a try/catch per item, logging a warning and continuing.
3. **Partial-batch success was discarded on a later library's failure.** The dispatcher sketched in §5.2 throws on the first non-2xx response within a multi-library batch, discarding `succeededKeys` already accumulated for libraries processed earlier in the same call — contradicting this section's own guarantee. Fixed by having the dispatcher catch per-library errors and resolve with `{ succeededKeys, failed }` instead of throwing; `DispatchResult` gained an optional `failed: boolean` flag so `_dispatchNext()` can apply backoff without discarding already-succeeded keys.
4. **`VectorStore.update_item_metadata()`/`get_item_chunks()` only scrolled a single Qdrant page** (`limit=1000`, no pagination), silently dropping chunks beyond that for any item with more chunks than one page — found via manual testing against a real 1584-chunk item. Both methods now paginate through the full result set. Unrelated to this feature's design, but touches the same endpoint this feature extends.
