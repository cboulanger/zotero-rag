...
// Directory holding per-service env files (may contain secrets like API keys).
// Overridable via ZOTERO_RAG_ENV_DIR for testing.
const SERVICE_ENV_DIR = process.env.ZOTERO_RAG_ENV_DIR || '/etc/zotero-rag';

/**
 * Resolve user-provided env specs to "KEY=VALUE" lines.
 * Name-only specs ("KEY") are looked up in process.env; inline specs ("KEY=VALUE")
 * pass through unchanged. Missing host vars are warned about and skipped.
 * @param {string[]|undefined} env
 * @returns {string[]}
 */
export function resolveEnvPairs(env) {
  const pairs = [];
  if (!env) return pairs;
  for (const spec of env) {
    if (spec.includes('=')) {
      ...
      const val = process.env[spec];
      if (val !== undefined) pairs.push(`${spec}=${val}`);
      else console.log(`[WARNING] Env var ${spec} not found in host environment, skipping`);
    }
  }
  return pairs;
...
/**
 * Write user-provided env vars (which may include secrets such as API keys) to a
 * root-only env file referenced by the systemd unit via podman's --env-file
 * (legacy ExecStart) / EnvironmentFile= (Quadlet). This keeps secret *values* out of
 * the unit file itself — previously they were inlined into `ExecStart=… -e KEY=VALUE`,
 * leaving them in plaintext on disk and visible via `systemctl cat` / process listings.
 *
 * Because podman re-reads the file on every container start, editing this file and
 * running `systemctl restart <service>` rotates a key WITHOUT a full redeploy.
 *
 * @param {string} serviceName
 * @param {string[]|undefined} env
 * @returns {string|null} absolute path to the env file, or null when there is nothing to write
 */
export function writeServiceEnvFile(serviceName, env) {
  const pairs = resolveEnvPairs(env);
  if (pairs.length === 0) return null;
  if (!fs.existsSync(SERVICE_ENV_DIR)) fs.mkdirSync(SERVICE_ENV_DIR, { recursive: true, mode: 0o700 });
  fs.chmodSync(SERVICE_ENV_DIR, 0o700);
  const envFilePath = `${SERVICE_ENV_DIR}/${serviceName}.env`;
  const header =
    '# Generated by container.mjs — deployment env vars (may include secrets).\n' +
    `# Edit values and \`systemctl restart ${serviceName}\` to rotate without redeploying.\n`;
  // podman --env-file ignores blank and #-comment lines.
  fs.writeFileSync(envFilePath, header + pairs.join('\n') + '\n', { mode: 0o600 });
...
export function buildQuadletContent(cfg, kreuzbergService, qdrantService, qdrantContainerName, envFilePath) {
  ...
  // User-provided env (may include secrets) comes from a root-only env file referenced
  // here (podman --env-file), so values never appear in this unit. Requires Podman 4.6+
  // for the Quadlet EnvironmentFile= key. Internal, non-secret extraEnv stays inline above.
  if (envFilePath) lines.push(`EnvironmentFile=${envFilePath}`);
...
export function buildLegacyUnitContent(cfg, kreuzbergService, qdrantService, qdrantContainerName, envFilePath) {
  ...
  // User-provided env (may include secrets) is loaded from a root-only env file via
  // --env-file, keeping values out of ExecStart. Internal, non-secret extraEnv stays inline.
...
function setupSystemdService(serviceName, cfg, kreuzberg, qdrant) {
  ...
  try {
    // Write user-provided env (API keys etc.) to a root-only 0600 file referenced by
    // the unit, instead of inlining secret values into ExecStart / Environment=.
    const envFilePath = writeServiceEnvFile(serviceName, cfg.env);
    if (envFilePath) {
      console.log(`[INFO] Wrote service env file (mode 0600): ${envFilePath}`);
      console.log(`[INFO] Rotate keys by editing it, then: systemctl restart ${serviceName}`);
    }
