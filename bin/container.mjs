 ...
 * @returns {string}
 */
function buildQuadletContent(cfg) {
...
/**
 * Write a Podman Quadlet .container file and enable the systemd service.
 * On every restart systemd creates a fresh container, eliminating zombie-container failures.
 * @param {string} serviceName
 * @param {Parameters<typeof buildQuadletContent>[0]} cfg
 * @returns {boolean}
 */
function setupSystemdService(serviceName, cfg) {
  console.log(`[INFO] Setting up systemd service '${serviceName}' via Quadlet...`);
  const quadletDir = '/etc/containers/systemd';
  const unitPath = `${quadletDir}/${serviceName}.container`;
  try {
    if (!fs.existsSync(quadletDir)) fs.mkdirSync(quadletDir, { recursive: true });
    fs.writeFileSync(unitPath, buildQuadletContent(cfg));
    console.log(`[INFO] Quadlet written to ${unitPath}`);
    execSync('systemctl daemon-reload', { stdio: 'inherit' });
    execSync(`systemctl enable --now ${serviceName}`, { stdio: 'inherit' });
    console.log(`[SUCCESS] Service '${serviceName}' enabled and started`);
    console.log(`[INFO] Status: systemctl status ${serviceName}`);
    console.log(`[INFO] Logs:   journalctl -u ${serviceName} -f`);
    return true;
  } catch (e) {
 ...
 *   nginx?: boolean, ssl?: boolean, email?: string, systemdService?: string, yes?: boolean
 * }} options
 */
async function handleDeploy(options) {
  ...
  if (options.systemdService) {
    // Systemd manages the container lifecycle via Quadlet — no manual `podman run` needed.
    // The Quadlet unit force-removes any zombie container before each start.
    if (!setupSystemdService(options.systemdService, containerCfg)) process.exit(1);
  } else {
    try {
...
Examples:
  ...
  # Deploy with systemd Quadlet service (auto-restarts, survives reboots)
  sudo env "PATH=$PATH" node bin/container.mjs deploy \\
    --fqdn rag.example.com --data-dir /srv/zotero-rag/data --pull \\
