...
class TestRequestResponseNewFields(unittest.TestCase):
    ...
    def test_diversity_tuning_fields_default_to_none(self):
        """None means "use rag_engine.py's hardcoded default" — see api/query.py's
        query_libraries(), which resolves None to the module-level constant."""
        from backend.api.query import QueryRequest

        req = QueryRequest(question="Q", library_ids=["1"])
        self.assertIsNone(req.diversity_floor)
        self.assertIsNone(req.diversity_escalation_factor)
        self.assertIsNone(req.diversity_escalation_max_top_k)
        self.assertIsNone(req.max_chunks_per_document)
        self.assertIsNone(req.low_diversity_available_floor)
