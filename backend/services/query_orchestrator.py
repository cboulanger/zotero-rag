...
from backend.services.rag_engine import (
    QueryResult, SourceInfo, _looks_like_tool_call_leak, _missing_citations, _low_citation_diversity,
    _LOW_DIVERSITY_AVAILABLE_FLOOR, _DIVERSITY_FLOOR, _DIVERSITY_ESCALATION_FACTOR,
    _DIVERSITY_ESCALATION_MAX_TOP_K, _MAX_CHUNKS_PER_DOCUMENT,
)
...
def _quality_issue_reinforcement(
    ...
    available_sources: number of distinct sources merged into context, used
    only by the low-diversity check below."""
    ...
    if _low_citation_diversity(answer, available_sources, low_diversity_floor):
        return (
            ...
            "sources were retrieved. Check whether any of the other sources in the "
            "retrieved information above also contain information relevant to the "
            "question, and cite them where genuinely relevant — do not cite a source "
            "that is not actually relevant just to add variety."
        )
    return None
...
class QueryOrchestrator:
    ...
    async def query(
        ...
        Args:
            ...
            diversity_floor, diversity_escalation_factor, diversity_escalation_max_top_k,
                max_chunks_per_document: Forwarded to RAGAgent -> RAGEngine.query() (see
    ...
    async def _synthesize(
        ...
        reinforcement = _quality_issue_reinforcement(answer, len(sources), low_diversity_available_floor)
        if reinforcement:
            logger.warning(
            ...
            if _quality_issue_reinforcement(answer, len(sources), low_diversity_available_floor):
                logger.warning(
