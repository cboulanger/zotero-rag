...
### Never kill/restart Zotero by name — always target the dev instance's PID

The user's **main personal Zotero instance** may be running at the same time
as the **zotero-rag development instance** used for live plugin/backend
testing. They are separate OS processes, distinguished only by the
`-profile` argument on the command line — **never** use `pkill -f zotero`,
`killall Zotero`, or any name-based match to stop/relaunch "the" Zotero
process. A name match hits both and can silently kill the user's main
instance and any unsaved work in it.

**Always resolve the dev instance's actual PID first**, and confirm it
before touching it:

```bash
ps aux | grep -i "Zotero.app/Contents/MacOS/zotero " | grep -v grep
```

The dev instance's command line includes `-profile "$ZOTERO_PLUGIN_PROFILE_PATH"`
(that env var is defined in `.env`, e.g. ending in
`.../Profiles/<id>.zotero-rag`). A Zotero process running **without** that
`-profile` flag, or with a different profile path, is the main instance —
leave it alone; do not kill it, do not restart it, do not touch it.

**If the dev instance's RDP bridge stops responding** (`zotero_ping` fails
with `ECONNREFUSED` on port 6100 — it can quit or crash independently of
anything you did), relaunch only that profile:

```bash
nohup "$ZOTERO_PLUGIN_ZOTERO_BIN_PATH" -profile "$ZOTERO_PLUGIN_PROFILE_PATH" \
  > data/logs/zotero_relaunch.log 2>&1 &
disown
```

(Both env vars come from `.env` — see "Running the backend + plugin dev
servers from a worktree" above.) The previous process's temporary plugin
add-on install is gone after a relaunch — restart the plugin dev server too
so it reinstalls into the new process:

```bash
nohup uv run python scripts/zotero_plugin.py start > data/logs/zplugin_dev.log 2>&1 &
disown
```

Wait for `zotero_ping` to succeed (and `Zotero.ZoteroRAG` to be defined via
`zotero_execute_js`) before issuing further `zotero_execute_js`/
`zotero_plugin_reload` calls — both can silently return `undefined` if
called too early.
